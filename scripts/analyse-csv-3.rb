...
#
# Algirithm is armed when weight has increased 10% from the last weight in range and is within the detection range
# AND the previous weight is less than empty (the teapot has left the scale)
#
...
#
#
# The only difference between this and algorithm 2 is that it assumes the teapot leaves the scales to fill
...
def modified_detection(reading)
  if (reading.to_f > (@data[:last_weight_in_range].to_f * 1.1)) and @data[:weight] < 0 and reading > 65 and reading < 110
    @armed = true
  end
...
@output_file = "#{File.dirname(__FILE__)}/../recorded_data/analysis-3.csv"
